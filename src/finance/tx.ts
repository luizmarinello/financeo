...
export async function addEntry(e: NewEntry): Promise<string[]> {
  ...
  const rows: Transaction[] = parts.map((amountCents, i) => {
    const month = addMonths(firstInvoice.month, i)
    return {
      id: uid(),
      type: e.type,
      amountCents,
      categoryId: e.categoryId,
      accountId: e.accountId,
      // a parcela i vive na data da compra deslocada para o mês da fatura dela
      date: i === 0 ? e.date : dateIn(month, dayOf(e.date)),
      description: e.description || undefined,
      cardId: e.cardId,
      invoiceMonth: month,
      purchaseId,
